...
def save_all_codes(run_path="run.py", train_path="train.py", dcgan_path="dcgan_model.py", output_file="all_codes.txt"):
    import os
...
def train_dcgan(device, nz, lr_g, lr_d, beta1,
                ...
                lambda_l1, lambda_mse, lambda_fm, update_d_every, dropout_p):   
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            #print(f"[Epoch {epoch+1} | Batch {i+1}] Real: {batch_size}, Fake: {fake_images.size(0)}")
            ...
            # --- Generatore ---
            netG.zero_grad()
            label_gen = torch.full((batch_size,), 1.0, device=device)
        ...
        with torch.no_grad():
            ...
            save_image(fake, f"{img_path}/epoch_{epoch + 1}.png", normalize=True)
            
            fake = netG(fixed_noise).detach().cpu()
            save_image(fake, f"{img_path}/final_result.png", normalize=True)

            npy_array = fake.numpy()
            ...
            if epoch + 1 >= 50 and epochs_no_improve >= early_stop_patience:
